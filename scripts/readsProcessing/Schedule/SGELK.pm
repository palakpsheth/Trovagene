...
sub new{
  ...
  my %default=(numnodes=>50,numcpus=>32,verbose=>0,waitForEachJobToStart=>0,maxslots=>9999,queue=>"testq",scheduler=>"SGE",pe=>"mpi1",keep=>1,seed=>1);
...
sub pleaseExecute{
  ...
  # add --force option to second run
  my $cmd2 = $cmd;
  $cmd2 = "$cmd2 --force";

  $self->waitOnJobs(\@jobsToMonitor,0); # wait until the job can be submitted
  
  # clean up jobname because SGE doesnt like jobnames that start with a number
  my $jname = $settings{jobname};
  #print "Original jobname: $jname\n";
  if ($jname =~ /(^[0-9]|^[0-9]+|^[0-9]_)/i) {
	while ($jname =~ /(^[0-9]|^-)/) {
		$jname =~ s/^[0-9]+//i;
		$jname =~ s/^-+//i;
	}
	$jname =~ s/^_+//i;
	$settings{jobname} = $jname;
	if ($settings{jobname} eq '') {
		$settings{jobname} = 'default_'.int(rand(99));
	}
	#print "\tNew jobname: $settings{jobname}\n";
  }
  ...
  srand($settings{seed});
  #my $gen = Math::Random::MT->new($settings{seed});
  #my $rand = int($gen->rand(9999999999999));
  my $rand = int(rand(999999));
  $rand = $settings{jobname} . "_$rand";
  ...
  if (!-d $tempdir) {
	  make_path $tempdir or die "ERROR: Could not make directory $tempdir: $!\n";
  }
  ...
  print SCRIPT "#\$ -pe $settings{pe} $settings{numcpus}\n";
  ...
  my $sanitized2=escape('qqbackslash',$cmd2);
...
if(\$? && \$? != 36096){
  ...
  print \$error;
  # try again
  write_file('$submitted',$sanitized2);
  # it runs the command
  write_file('$running',$sanitized2);
  system($sanitized2);
}

END
 # print SCRIPT "system($sanitized2) if \$?;\n";
  # print a parsable error if the script dies. This error will be in $output and in file $died
  print SCRIPT <<END;
if(\$? && \$? != 36096){
  my \$error=\"QSUB ERROR\\n\$?\\n\$!\";
  write_file('$died',$sanitized2);
  write_file('$died',{append=>1},"\\n\$error\\n");
  print \$error;
}
END
